...
python /code/picsgrabber_src/manage.py collectstatic --noinput
python /code/picsgrabber_src/manage.py syncdb --noinput
python /code/picsgrabber_src/manage.py makemigrations
python /code/picsgrabber_src/manage.py migrate --noinput
...
if [ "$DJANGO_PRODUCTION" != "true" ]; then
    ...
    if [ "$ROOT_PASSWORD" == "" ]; then
        ...
        echo "from ConfigParser import SafeConfigParser; parser = SafeConfigParser(); parser.read('/code/docker_files/config.ini'); from django.contrib.auth.models import User; print 'Root user already exists' if User.objects.filter(username='root') else User.objects.create_superuser('root', 'admin@example.com', parser.get('general', 'ROOT_PASSWORD'))" | python /code/picsgrabber_src/manage.py shell
    else
        # Root password environment variable IS set; so, use it
        echo "import os; from django.contrib.auth.models import User; print 'Root user already exists' if User.objects.filter(username='root') else User.objects.create_superuser('root', 'admin@example.com', os.environ['ROOT_PASSWORD'])" | python /code/picsgrabber_src/manage.py shell
    fi
else
